...
//    [self.view addSubview:btnFirst];
    ...
    [btnSecond setImage:[UIImage imageNamed:@"btn_main_p"] forState:UIControlStateNormal];
...
//    [self.view addSubview:labelFirst];
    ...
    labelSecond.frame = CGRectMake((ScreenWidth-45) / 2, 529+5, 72, 18);
    labelSecond.text = @"确定";
...
- (IBAction)backToViewController:(UIButton *)sender{
    // TODO
    UIImage *showImage = [[BDFaceImageShow sharedInstance] getSuccessImage];
    NSData *imgData = UIImageJPEGRepresentation(showImage, 1.0f);
         
    NSString *encodedImageStr = [imgData base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
        // TODO
    //BdTransFlutterStreamManager.sharedInstance.streamHandler.eventSink(encodedImageStr);
    
    LbwBDFacePlugin.myresult(encodedImageStr);
   // [LbwBDFacePlugin myresult](encodedImageStr);
    
    [self dismissViewControllerAnimated:YES completion:^{
            　　//关闭传值
        [[NSNotificationCenter defaultCenter] postNotificationName:@"yiJiBackTo" object:self userInfo:[NSDictionary dictionaryWithObject:encodedImageStr forKey:@"yiJiBackToVal"]];
    }];
